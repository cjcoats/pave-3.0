 ...
 *  PAVE Version 3.0
 *
 *  File: $Id: utils.noioapi.c 83 2018-03-12 19:24:33Z coats $
...
int range_get (  char *percents, /* array to denote which cells in domain */
    ...
    *jmax = 0;
    ...
    if ( !on ) return ( errmsg (
                                "There are no selected cells in this formula's region of interest!" ) );
...
#ifdef HACKHACKHACK /* SRT change tbis to look for IEEE NaN ! */
            if ( percents[INDEX ( i /* +vdata->col_min-1 removed SRT 2/23/95 */ ,
                                  j /* +vdata->row_min-1 removed SRT 2/23/95 */ ,
                                  0,
                                  0,
                                  IMAX,
                                  JMAX,
...
void julian2shorttext ( char *string, int date, int time ) /* SRT 101496 added */
    ...
    */
    ...
    for ( i = 0; ( ( i < 12 ) && ( !found ) ); ++i )
        ...
        if ( day <= j )
            {
            sprintf ( string, "%d/%d/%d %d:%02d:%02d", /* SRT removed 2 spaces before th
e time 960412 */
